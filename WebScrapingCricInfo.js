...
// npm install pdf-lib
// node WebScrapingCricInfo.js --htmlFile="download.html" --excel=Worldcup.csv --dataDir=worldcup --source=https://www.espncricinfo.com/series/icc-cricket-world-cup-2019-1144415/match-results
...
function prepareExcel(teams, excelFileName) {
    ...
    for (let i = 0; i < teams.length; i++) {
        ...
        tsheet.cell(1, 1).string("Vs").style(hstyle);
        tsheet.cell(1, 2).string("Self Score").style(hstyle);
        tsheet.cell(1, 3).string("Opp Score").style(hstyle);
        tsheet.cell(1, 4).string("Result").style(hstyle);
        for (let j = 0; j < teams[i].matches.length; j++) {
